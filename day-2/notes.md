...
3. Paste contents of our script from VS Code to open file
 > deploy_nginx.sh
...
8a. If you are denied permission to run the file you can try:
Adding permissions to execute by running the command:
`sudo chmod +x nginx-deploy.sh`
8b. Run the previous command again
...
==============================================================
npm is the package manager for node.js packages/modules (Javascript libraries). When you install nodejs, it also installs npm.
...
`npm start app.js`
nb. npm start can be run with a relative directory
...
4. You should see the nginx homepage

nb. As the app is running on port 3000, not 80 (the default)
4. Add `:3000` to the IP address
